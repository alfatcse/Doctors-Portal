...
const AvailableAppointment = ({ selectedDate }) => {
   // const [appointmentOptions, setappointmentOptions] = useState([]);
    ...
    const {data:appointmentOptions=[],refetch,isLoading}=useQuery({
        ...
        queryFn:async()=>{
            const res=await fetch(`https://doctors-portal-server-blush-psi.vercel.app/appointmentOptions?date=${date}`);
    ...
    return (
        <section className='mt-16'>
            ...
            {
                treatment&&
                <BookingModal selectedDate={selectedDate} refetch={refetch} setTreatment={setTreatment} treatment={treatment}></BookingModal>
            }
        </section>
    );
};
