...
const AllPatient = () => {
    ...
    return (
        <div>
            <h1 className='text-center font-bold mb-3 text-blue-600'>Number of Patients: {users?.length}</h1>
                    ...
                    <tbody>
                        {
                            users?.map((user, i) =>
                                <tr key={user._id}>
                                    ...
                                    <td>
                                        <label onClick={() => setDeletingDoctor(user)} htmlFor="confirmation-modal" className="btn btn-circle btn-outline hover:bg-red-500"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></label>
                                    </td>
                                </tr>
                            )
                        }
